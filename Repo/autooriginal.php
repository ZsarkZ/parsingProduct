   ...
   public function find($article, $brand = '')
	{
        ...
        if(strcasecmp($article, $valid) == 0){
         foreach ($links as $link) {
                 $manufacturer = 'Производитель : '.pq($link)->find('td:eq(1)').'<br>';
                 $number = 'Код                 : '.pq($link)->find('td:eq(2)').'<br>';
                 $name = 'Наименование          : '.pq($link)->find('td:eq(3)').'<br>';
		...
		$result[$this->siteUrl] = array(
			'Origin' => array(
                          'manufacturer'=>$manufacturer,'number'=>$number,'name'=>$name,'price'=>$price,'available'=>$available,'order_links'=>$order_links
                ), 
